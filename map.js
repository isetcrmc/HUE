...
// Base map Google
const baseLayers = {
  "Roadmap": L.tileLayer('https://mt1.google.com/vt/lyrs=r&x={x}&y={y}&z={z}', { attribution: "Google Roadmap" }),
  "Satellite": L.tileLayer('https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}', { attribution: "Google Satellite" })
};
...
function addFloodLayer(year, color) {
  fetch("Flood_trace_all.geojson").then(res => res.json()).then(data => {
    const layer = L.geoJSON(data, {
      ...
      onEachFeature: (f, l) => {
        let p = f.properties;
        let popup = `<b>${p.Name}</b><br><b>Code:</b> ${p.Code}<br><b>Vị trí:</b> ${p.Commune}, ${p.District}<br><b>Tọa độ:</b> ${p.X}, ${p.Y}`;
    ...
    overlays["Vết lũ"][`Vết lũ ${year}`] = layer;
    layer.addTo(map);
  });
}
addFloodLayer('2020', 'orange');
addFloodLayer('2022', 'yellow');
addFloodLayer('2023', 'green');
  ...
  types.forEach(type => {
    const layer = L.geoJSON(data, {
      ...
      onEachFeature: (f, l) => {
        ...
        const popup = `<b>${p.Name2 || p.Name}</b><br><b>Loại:</b> ${p.Type}<br><b>Vị trí:</b> ${p.Name}<br><b>Tọa độ:</b> ${p.X}, ${p.Y}`;
        l.bindPopup(popup);
      }
    });
